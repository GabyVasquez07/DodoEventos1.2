    ...
    <form action="{{ route('guardar') }}" method="POST">
        ...
        <select name="fk_usuario">
            @foreach(($organizadores) as $organizador)
              <!--<option value="{{ $organizador->id }} {{$event->fk_usuario == $organizador->id ? 'selected':''}}">{{ $organizador->nombre }}</option>-->
            @endforeach
        </select>  <br><br>

        <label for="nombre">Nombre del evento:</label>
       <!-- <input type="text"  name="nombre_evento" value="{{$event->nombre_evento" required ><br><br>-->

        <label for="descripcion">Descripción:</label>
      <!--  <input type="text" name="descripcion" value="{{$event->descripcion" required><br><br>-->
